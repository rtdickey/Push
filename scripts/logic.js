...
GameState = function() {
  ...
  this.translateLeft = function(x, y) {
    ...
    if((i === 0 && y === 0) || 
       (i === 0 && y === 6) ||
       (i === 0 && y === 7)) {
      if(this.board[x][y].color !== this.board[i+1][y].color)
        throw this.board[x][y].color + ' wins the game!';
      else
        throw this.board[i+1][y].color + ' loses the game!';
    }
    throw 'no empty space! invalid push!';
  },
